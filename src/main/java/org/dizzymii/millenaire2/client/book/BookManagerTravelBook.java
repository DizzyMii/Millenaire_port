...
public class BookManagerTravelBook {
    ...
    public static Map<String, List<ClientPacketHandler.VillageListClientEntry>> indexByCulture(
            List<ClientPacketHandler.VillageListClientEntry> entries) {
        Map<String, List<ClientPacketHandler.VillageListClientEntry>> byCulture = new HashMap<>();
        for (ClientPacketHandler.VillageListClientEntry entry : entries) {
            String culture = nonNull(entry.cultureKey, "unknown");
            byCulture.computeIfAbsent(culture, k -> new ArrayList<>()).add(entry);
        }
        for (List<ClientPacketHandler.VillageListClientEntry> cultureEntries : byCulture.values()) {
            cultureEntries.sort(Comparator.comparingInt(v -> v.distance));
        }
        return byCulture;
    }

    public static TextBook generateHomeBook(List<ClientPacketHandler.VillageListClientEntry> cache) {
        ...
        Map<String, List<ClientPacketHandler.VillageListClientEntry>> byCulture = indexByCulture(cache);
        ...
        List<ClientPacketHandler.VillageListClientEntry> nearest = new ArrayList<>(cache);
        nearest.sort(Comparator.comparingInt(v -> v.distance));
        int max = Math.min(8, nearest.size());
        for (int i = 0; i < max; i++) {
            ClientPacketHandler.VillageListClientEntry e = nearest.get(i);
        ...
        for (String culture : cultures) {
            List<ClientPacketHandler.VillageListClientEntry> list = byCulture.get(culture);
            if (list == null) continue;
            int villages = 0;
            int lone = 0;
            int nearestDist = Integer.MAX_VALUE;
            for (ClientPacketHandler.VillageListClientEntry e : list) {
    ...
    public static TextBook generateCultureBook(String cultureKey, List<ClientPacketHandler.VillageListClientEntry> cache) {
        ...
        for (ClientPacketHandler.VillageListClientEntry e : cache) {
        ...
        int count = 0;
        for (ClientPacketHandler.VillageListClientEntry e : filtered) {
            String name = nonNull(e.name, "Unknown");
    ...
    public static TextBook generateVillageDetailBook(ClientPacketHandler.VillageListClientEntry entry) {
