...
public class MillGoalBehaviour extends ExtendedBehaviour<MillVillager> {
    ...
    private static final long TRAVEL_TIMEOUT_TICKS = 600; // 30 seconds max travel time
    private static final long GOAL_COOLDOWN_TICKS = 100; // 5 seconds between same goal
    ...
    protected void start(ServerLevel level, MillVillager villager, long gameTime) {
        ...
        // Call onAccept callback (mirrors original setNextGoal)
        try {
            best.onAccept(villager);
        } catch (Exception e) {
            MillLog.error(villager, "Error in goal onAccept: " + best.key, e);
        }
    ...
    protected void tick(ServerLevel level, MillVillager villager, long gameTime) {
        ...
        if (activeInfo.hasTarget() && !arrivedAtTarget) {
            ...
            if (dist > (range * range + 1)) {
                // Travel timeout — if we've been trying to reach dest too long, abort
            ...
            villager.stopMoving = true; // Stop moving while performing action
    ...
    private Goal selectBestGoal(MillVillager villager) {
        ...
        for (String goalKey : villager.vtype.goals) {
            ...
            // Cooldown check — don't re-select same goal too quickly
