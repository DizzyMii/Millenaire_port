...
public class DefendVillageBehaviour extends ExtendedBehaviour<MillVillager> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel level, MillVillager villager) {
        ...
        this.target = villager.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).orElse(null);
    ...
    protected void start(ServerLevel level, MillVillager villager, long gameTime) {
        ...
        villager.getBrain().setMemory(MillMemoryTypes.ACTIVE_GOAL_KEY.get(), "defendvillage");
        // Walk toward target
    ...
    protected boolean shouldKeepRunning(MillVillager villager) {
        LivingEntity t = villager.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).orElse(null);
